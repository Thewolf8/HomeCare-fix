...
import {
  ArrowLeft, Camera, X, Package, ChevronDown, Plus, Minus
} from 'lucide-react';
import { useApp, roomToKey } from '@/hooks/useAppContext';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';
import { Textarea } from '@/components/ui/textarea';
import type { Category, Room, UnitType } from '@/types';
...
export default function EditProduct() {
  ...
  useEffect(() => {
    if (product) {
      ...
      setSelectedRooms(product.rooms);
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="sticky top-0 z-40 bg-background/80 backdrop-blur-xl border-b border-border">
        <div className="max-w-3xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <h1 className="text-lg font-semibold">{t('editProduct')}</h1>
          <div className="w-14" />
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-3xl mx-auto px-4 py-6 space-y-6">
          ...
          >
            {photo ? (
              <>
                ...
                <button type="button" onClick={(e) => { e.stopPropagation(); setPhoto(''); }} className="absolute top-1 right-1 p-1 rounded-full bg-background/80">
                  <X className="w-3 h-3" />
                </button>
              </>
            ) : (
        ...
        <div className="space-y-2">
          ...
          <button type="button" onClick={() => setShowCategoryPicker(!showCategoryPicker)}
          ...
          {showCategoryPicker && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="grid grid-cols-2 gap-2 overflow-hidden">
              {categories.map(cat => (
                <button key={cat} type="button" onClick={() => { setCategory(cat); setShowCategoryPicker(false); }}
                  className={`p-3 rounded-xl text-sm font-medium transition-all ${category === cat ? 'bg-primary text-primary-foreground' : 'bg-muted hover:bg-accent'}`}>
                  {t(cat)}
                </button>
              ))}
            </motion.div>
          ...
          <div className="flex flex-wrap gap-2">
            {allRooms.map(room => (
              <button key={room} type="button" onClick={() => toggleRoom(room)}
                className={`px-3 py-2 rounded-xl text-sm font-medium transition-all ${selectedRooms.includes(room) ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:bg-accent'}`}>
        ...
        <div className="space-y-2">
          ...
          <Input value={brand} onChange={(e) => setBrand(e.target.value)} className="h-12 rounded-xl" />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <div className="flex items-center h-12 rounded-xl border border-input bg-background">
              <button type="button" onClick={() => setQuantity(Math.max(0, quantity - 1))} className="p-3 hover:bg-accent rounded-l-xl transition-colors"><Minus className="w-4 h-4" /></button>
              <Input type="number" value={quantity} onChange={(e) => setQuantity(Math.max(0, parseInt(e.target.value) || 0))} className="h-full border-0 text-center bg-transparent" />
              <button type="button" onClick={() => setQuantity(quantity + 1)} className="p-3 hover:bg-accent rounded-r-xl transition-colors"><Plus className="w-4 h-4" /></button>
            </div>
          </div>
          <div className="space-y-2">
            <label className="text-sm font-medium">{t('unit')}</label>
            <button type="button" onClick={() => setShowUnitPicker(!showUnitPicker)}
              className="w-full h-12 px-4 rounded-xl border border-input bg-background flex items-center justify-between hover:border-primary/50 transition-colors">
        ...
        {showUnitPicker && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="flex flex-wrap gap-2 overflow-hidden">
            {units.map(u => (
              <button key={u} type="button" onClick={() => { setUnit(u); setShowUnitPicker(false); }}
                className={`px-3 py-2 rounded-xl text-sm font-medium transition-all ${unit === u ? 'bg-primary text-primary-foreground' : 'bg-muted hover:bg-accent'}`}>
                {t(u + '_unit' as any) || u}
              </button>
            ))}
          </motion.div>
        ...
        <div className="space-y-2">
          ...
          <Input type="number" value={minThreshold} onChange={(e) => setMinThreshold(Math.max(0, parseInt(e.target.value) || 0))} className="h-12 rounded-xl" min={0} />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Input type="date" value={expirationDate} onChange={(e) => setExpirationDate(e.target.value)} className="h-12 rounded-xl" />
          </div>
          <div className="space-y-2">
            <label className="text-sm font-medium">{t('purchaseDate')} <span className="text-muted-foreground">({t('optional')})</span></label>
            <Input type="date" value={purchaseDate} onChange={(e) => setPurchaseDate(e.target.value)} className="h-12 rounded-xl" />
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <Input type="number" step="0.01" value={pricePerUnit} onChange={(e) => setPricePerUnit(e.target.value)} className="h-12 rounded-xl" />
        </div>

        {/* Notes */}
        <div className="space-y-2">
          <label className="text-sm font-medium">{t('notes')} <span className="text-muted-foreground">({t('optional')})</span></label>
          <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="min-h-[100px] rounded-xl resize-none" />
        </div>

        {/* Submit */}
        <div className="pt-4 pb-8">
          <Button type="submit" className="w-full h-14 rounded-xl bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-600 hover:to-cyan-600 text-white shadow-lg shadow-teal-500/20 text-base font-semibold">
